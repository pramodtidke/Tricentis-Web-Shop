 ...
 *
 * - Reads cart items from Zustand global state
 * - Increase / decrease quantity buttons (auto-removes at 0)
 * - Remove item button
 * - Running total and item count
 * - "Proceed to Checkout" navigates to /checkout
 * - Animated slide-in from the right (CSS transition, no extra deps)
 * - Closes on backdrop click or the close button
 *
 * Usage: Place <CartDrawer /> once in your root layout.
 * Toggle it from anywhere: useStore(s => s.toggleCart)()
 */
...
export default function CartDrawer() {
  const { items, isCartOpen, closeCart, increaseQuantity, decreaseQuantity, removeItem } =
    useStore((s) => s);
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full pt-20 text-center gap-4">
      ...
      <div>
        ...
        <p className="text-sm text-slate-400 mt-1">Add some items to get started.</p>
      </div>
    </div>
  );
}
